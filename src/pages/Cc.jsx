...
const CreateCheck = () => {
  ...
  useEffect(() => {
    ...
    const fetchNextCheckNumber=async()=>{
      try{
        const response=await axios.get('http://localhost:8080/api/checks/next-check-number',{
          headers:{
            'Authorization':`Bearer ${token}`
          }
      });
      setFormData(prevFormData=>({...prevFormData,checkNumber:response.data}));
      }catch(error){
  ...
  <Grid item xs={12} md={6}>
    <Grid container spacing={2} justifyContent="flex-end">
      <Grid item>
        <Button type="button" variant="outlined" onClick={handlePreview} sx={{ border: '1px solid black', color: 'black' }}>Preview</Button>
      </Grid>
      <Grid item>
    ...
    <Dialog open={isDialogOpen} onClose={handleClose}>
      ...
      {isPreviewVisible && <Preview open={isPreviewVisible} formData={formData} />}
      {/* <h1>YEAH PREVIEW</h1> */}
